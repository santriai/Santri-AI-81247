...
const EbookReaderScreen: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { user } = useAuth();
  const { showToast } = useToast();
  
  const state = location.state as { bookId: string, title: string, author: string, coverImage?: string };
  ...
  useEffect(() => {
    if (user) {
      const unsubscribe = subscribeUserBookmarks(user.uid, 'ebook', (items) => {
        setBookmarks(items);
      });
      return () => unsubscribe();
    }
  ...
  const handleBookmark = async (chapter: Chapter) => {
    ...
    const bookmarkId = `ebook-${data.profile.name}-${chapter.title}`.replace(/\s+/g, '-').toLowerCase();
    ...
    if (existing) {
       ...
       await saveUserBookmark(user.uid, 'ebook', bookmarkId, {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const result = await fetchEbookChapters(state.title, state.author);
        setData(result);
      } catch (error) {
        console.error("Gagal memuat konten ebook:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-[#2c3e50] flex flex-col items-center justify-center p-6 text-center">
        <p className="text-white mb-4">Gagal memuat isi kitab.</p>
  ...
  return (
    <div className="min-h-screen bg-[#1a252f] pb-20 font-sans">
      {/* Header */}
      <div className="sticky top-0 z-50 bg-[#2c3e50] border-b border-white/10 px-4 py-4 flex items-center gap-4 shadow-lg">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6">
        {/* Book Profile Section - Styled like Image 3 */}
        <div className="bg-[#2c3e50] p-6 rounded-2xl border border-white/5 shadow-2xl mb-8 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-4">
             <div className="flex flex-col items-center mb-6">
                <div className="w-24 h-32 bg-gradient-to-br from-[#f1c40f] to-[#d35400] rounded-lg shadow-2xl flex items-center justify-center mb-4 border-2 border-white/20 overflow-hidden">
                   {state.coverImage ? (
                     <img src={state.coverImage} alt={state.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                   ) : (
                     <BookIcon size={40} className="text-[#2c3e50]" />
                   )}
                </div>
             ...
             <div className="space-y-3">
                ...
                <ProfileItem label="Nama di Dunia Arab" value={data.profile.originalTitle} isArabic />
                <ProfileItem label="Penerjemah" value={data.profile.translator || '-'} />
                <ProfileItem 
                  label="Bidang Studi" 
                  value={data.profile.field} 
                  onClick={() => navigate('/category-books', { state: { id: data.profile.field.toLowerCase(), label: data.profile.field } })}
                />
             </div>
          </div>
        </div>
        ...
        {data.profile.about && (
          <div className="bg-[#2c3e50]/50 p-6 rounded-2xl border border-white/5 shadow-inner mb-8 transition-all hover:bg-[#2c3e50]/80">
            <h3 className="text-[#f1c40f] text-[10px] font-black uppercase tracking-[0.3em] mb-3 flex items-center gap-2">
              <BookIcon size={14} /> Tentang Kitab
            </h3>
        ...
        {/* Accordion Content - Styled like Image 1 & 2 */}
                ...
                {expandedIndex === index && (
                  ...
                  >
                    <div className="p-6 md:p-8 space-y-8 flex flex-col items-center">
                       ...
                       <div className="w-full border-t border-[#2c3e50]/10 pt-4">
                          ...
                          <p className="text-[#2c3e50] text-lg md:text-xl font-medium leading-relaxed text-center px-2 mb-6">
                             {chapter.translation}
                          </p>
